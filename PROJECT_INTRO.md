...
- **东方财富 `index_global_hist_em`**（通过 akshare 调用）：美元指数(DXY)历史日线数据，备用 Yahoo Finance

### 2. 新闻数据

通过 **18 个 RSS 订阅源** 全天候抓取全球大宗商品与地缘政治新闻：
...
| 指标 | 数据源 | 说明 |
|------|--------|------|
| 美元指数 (DXY) | 东方财富 (akshare) | ICE 美元指数日线，备用 Yahoo Finance |
...
| `fetch_news_data` | 15 分钟 | 抓取 18 个 RSS 源新闻，去重入库 |
